...
internal class ComplexTest {
    ...
    operator fun unaryMinus() {
        //assertApproxEquals(Complex(1.0, -2.0), -Complex(2.0, -1.0), 1e-10) Ошибка ПРЕПА - тупой доцент
        assertApproxEquals(Complex(-2.0, 1.0), -Complex(2.0, -1.0), 1e-10)
    }

    @Test
    fun minus() {
        //assertApproxEquals(Complex("4-2i"), Complex("1+2i") + Complex("3-4i"), 1e-10)  Ошибка ПРЕПА - тупой доцент
        assertApproxEquals(Complex("-2+6i"), Complex("1+2i") - Complex("3-4i"), 1e-10)
    }

    @Test
    fun times() {
        //assertApproxEquals(Complex("11-8i"), Complex("1+2i") + Complex("3-4i"), 1e-10)   Ошибка ПРЕПА - тупой доцент
        assertApproxEquals(Complex("11+2i"), Complex("1+2i") * Complex("3-4i"), 1e-10)
    }

    @Test
    fun div() {
        //  assertApproxEquals(Complex("1+2i"), Complex("11-8i") + Complex("3-4i"), 1e-10)  Ошибка ПРЕПА - тупой доцент
