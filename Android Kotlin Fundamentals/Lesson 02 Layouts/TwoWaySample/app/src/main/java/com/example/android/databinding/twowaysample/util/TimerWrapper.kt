 ...
 * Таймер, используемый приложением, настраивается. Таким образом, тесты могут выполняться синхронно и очень быстро.
 *
 * See [DefaultTimer] for the default implementation and IntervalTimerViewModelTest.kt for a
 * test implementation.
 * См. раздел [таймер по умолчанию] для проверки реализации по умолчанию и интервального таймера ViewModel.КТ для
 * тестовая реализация.
 */
interface Timer {
 ...
 * Таймер по умолчанию используется при обычном выполнении приложения.
 */
