 ...
 * Список результатов запроса хранится в памяти GithubRepository в a ConflatedBroadcastChannel
 * и отображается как Flow.
 *
 * В настоящее время на GithubRepository него возложено множество функций источника данных,
 * которые библиотека подкачки будет обрабатывать после того, как мы его добавим:
 *
 * Загружает данные из GithubService, гарантируя, что несколько запросов не будут инициированы одновременно.
 * Сохраняет кэш извлеченных данных в памяти.
 * Отслеживает запрашиваемую страницу.
 *
 */
