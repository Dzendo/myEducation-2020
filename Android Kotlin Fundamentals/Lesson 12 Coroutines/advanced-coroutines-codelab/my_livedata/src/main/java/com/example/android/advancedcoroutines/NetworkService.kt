...
class NetworkService {
    ...
    // Считывает с сервера предпочтительные растения и выделяет в список их plantId
    suspend fun customPlantSortOrder(): List<String> = withContext(Dispatchers.Default) {
        val result = sunflowerService.getCustomPlantSortOrder() // suspend fun чтения с сети см ниже прямо из GitHub
