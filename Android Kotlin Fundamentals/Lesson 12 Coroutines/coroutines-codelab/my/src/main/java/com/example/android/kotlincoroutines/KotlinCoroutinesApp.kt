...
class KotlinCoroutinesApp : Application() {
     ...
     *
     * Use it to setup any background tasks,
     * running expensive setup operations in a background thread
     * to avoid delaying app start.
     * Используйте его для настройки любых фоновых задач,
     * выполнение дорогостоящих операций настройки в фоновом потоке
     * чтобы избежать задержки запуска приложения.
     */
    ...
    private fun setupWorkManagerJob() {
        ...
        // Используйте ограничения, чтобы требовать выполнения работы только тогда, когда устройство заряжается и сеть не отключена
        ...
        // Enqueue it work WorkManager, сохраняя все ранее запланированные задания для одной и той же работы.
