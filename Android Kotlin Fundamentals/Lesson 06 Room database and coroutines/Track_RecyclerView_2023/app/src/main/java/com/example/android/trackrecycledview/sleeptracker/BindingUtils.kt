...
Чтобы получить некоторые списки управления бесплатно, вы можете использовать ListAdapterкласс вместо RecyclerView.Adapter. Однако, если вы используете, ListAdapterвы должны написать свой собственный адаптер для других макетов, поэтому эта кодовая метка показывает вам, как это сделать.
Чтобы открыть меню намерений в Android Studio, поместите курсор на любой элемент кода и нажмите Alt+Enter( Option+Enterна Mac). Это меню особенно полезно для рефакторинга кода и создания заглушек для реализации методов. Меню является контекстно-зависимым, поэтому вам нужно разместить курсор точно, чтобы получить правильное меню.
Привязка данных:
