...
class GameOverFragment : Fragment() {
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
       ...
       // Add OnClick Handler for Try Again button
        // Добавить обработчик OnClick для кнопки повторить попытку
        binding.tryAgainButton.setOnClickListener{view: View->
            view.findNavController()
        //          .navigate(R.id.action_gameOverFragment_to_gameFragment)}
        // передайте GameOverFragmentDirections.actionGameOverFragmentToGameFragment()в качестве navigate()аргумента метода:
