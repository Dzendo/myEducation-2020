 ...
 * Фрагмент, используемый для того, чтобы показать, как глубокая ссылка на пункт назначения
 * Много чего делает, но и посылает уведомление с бляком и входи из этого уведомления сюда
 * Сюда же приход от виджета и от ссылки через гоогл
 * И это еще частично через провыйдер
 */
