 ...
 * Для простоты тестируется только одна функция: таймер запускается и приостанавливается в определенное время.
 * Затем тесты проверяют правильность выставленного времени.
 */
class IntervalTimerViewModelTest {
    ...
    fun timePerWork_afterMoreThanOneWorkCycle() {
        ...
        // Создайте ViewModel с таймером, который останавливается после полного рабочего набора и немного больше.
        ...
        // Убедитесь, что таймер предоставляет правильные данные
    ...
    private fun testWorkTimeLeftAfterTicks(tenths: Int) {
        ...
        // Создайте ViewModel с таймером, который останавливается через некоторое время.
 ...
 * a real clock.
 * Таймер, используемый для тестов, которые выполняют задачу несколько раз.
 * Он использует палочки вместо того, чтобы настоящие часы.
 */
