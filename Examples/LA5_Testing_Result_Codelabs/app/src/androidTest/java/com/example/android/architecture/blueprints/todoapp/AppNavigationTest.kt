...
/**
 * Tests for the [DrawerLayout] layout component in [TasksActivity] which manages navigation within the app.
 * Тесты для компонента макета [DrawerLayout] в [Tasks Activity], который управляет навигацией внутри приложения.
 *
...
class AppNavigationTest {
     ...
     * Это необходимо, когда операции не запланированы в основном Петлителе
     * (например, при выполнении в другом потоке).
     */
    @Before
     ...
     * Отмените регистрацию вашего ресурса холостого хода,
     * чтобы он мог быть собран мусором и не пропускал никакой памяти.
     */
    @After
    ...
    fun tasksScreen_clickOnAndroidHomeIcon_OpensNavigation() {
        ...
        // Проверка того, что панель навигации открыта
        onView(withId(R.id.drawer_layout))
    ...
    fun taskDetailScreen_doubleUpButton() = runBlocking {
        ...
        // Нажатие кнопки навигации на панели инструментов
        onView(
            withContentDescription(
                activityScenario
                    .getToolbarNavigationContentDescription()
...
// Следующая функция расширения использует панель инструментов getNavigationContentDescription
// для получения описания содержимого этого значка - ьутерброда.
